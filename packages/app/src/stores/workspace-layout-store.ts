...
function normalizeWorkspaceTab(value: unknown): WorkspaceTab | null {
  ...
  const target = normalizeTabTarget(tab.target);
  const tabId = trimNonEmpty(tab.tabId) ?? (target ? buildDeterministicTabId(target) : null);
  if (!target || !tabId) {
    return null;
  }
...
function normalizeSizes(input: { sizes: number[]; count: number }): number[] {
  if (input.count <= 0) {
    return [];
  }
...
function insertChildIntoGroup(
  ...
  input: { index: number; node: SplitNodeInternal; sizes: number[] }
): SplitNodeInternal {
...
function normalizeLayout(layout: unknown): WorkspaceLayout {
...
function reorderTabsForPane(input: { pane: SplitPaneInternal; tabIds: string[] }): SplitPaneInternal {
...
function detachTabFromTree(
  ...
  input: { tabId: string; preserveEmptyPaneId?: string | null }
): { root: SplitNodeInternal; tab: WorkspaceTab | null; sourcePaneId: string | null } {
...
function insertTabIntoPane(
  root: SplitNodeInternal,
  input: { paneId: string; tab: WorkspaceTab; focusTabId?: string | null }
): SplitNodeInternal {
...
function updateTabInTree(
  root: SplitNodeInternal,
  input: { tabId: string; target: WorkspaceTabTarget }
): SplitNodeInternal {
  const panePath = findPanePathContainingTab(root, input.tabId);
...
function updateGroupSizesInTree(
  root: SplitNodeInternal,
  input: { groupId: string; sizes: number[] }
): SplitNodeInternal {
...
function updatePaneInTree(
  root: SplitNodeInternal,
  input: { paneId: string; updater: (pane: SplitPaneInternal) => SplitPaneInternal }
): SplitNodeInternal {
  const panePath = findPanePathById(root, input.paneId);
  if (!panePath) {
    return root;
...
function insertSplitInternal(
  root: SplitNodeInternal,
  targetPaneId: string,
  tabId: string,
  position: "left" | "right" | "top" | "bottom"
): { root: SplitNodeInternal; newPaneId: string } {
  const direction = position === "left" || position === "right" ? "horizontal" : "vertical";
  const insertAfter = position === "right" || position === "bottom";

  const targetPathBeforeDetach = findPanePathById(root, targetPaneId);
  invariant(targetPathBeforeDetach, `Target pane not found: ${targetPaneId}`);

  const detached = detachTabFromTree(root, { tabId, preserveEmptyPaneId: targetPaneId });
  invariant(detached.tab, `Tab not found: ${tabId}`);

  const targetPath = findPanePathById(detached.root, targetPaneId);
  invariant(targetPath, `Target pane not found after detach: ${targetPaneId}`);
  const targetNode = getNodeAtPath(detached.root, targetPath);
  invariant(targetNode.kind === "pane", "Expected target pane after detach");

  const newPaneId = generateNodeId("pane");
...
export function insertSplit(
  ...
  return insertSplitInternal(asInternalNode(root), targetPaneId, tabId, position).root;
}
  ...
  persist(
    (set, get) => ({
      ...
      openTab: (workspaceKey, target) => {
        ...
        const layout = asInternalLayout(getWorkspaceLayout(get().layoutByWorkspace, normalizedWorkspaceKey));
        const existingTab = collectAllTabs(layout.root).find((tab) => tabTargetsEqual(tab.target, normalizedTarget));
        if (existingTab) {
          get().focusTab(normalizedWorkspaceKey, existingTab.tabId);
        ...
        const nextRoot = insertTabIntoPane(layout.root, {
      ...
      closeTab: (workspaceKey, tabId) => {
        ...
        set((state) => {
          const layout = asInternalLayout(getWorkspaceLayout(state.layoutByWorkspace, normalizedWorkspaceKey));
          const pane = findPaneContainingTab(layout.root, normalizedTabId);
          if (!pane) {
            return state;
          }

          const fallbackPaneId = findNearestSiblingPaneId(layout.root, pane.id);
          const nextRoot = removeTabFromTree(layout.root, normalizedTabId) as SplitNodeInternal;
          const nextFocusedPaneId =
            findPaneById(nextRoot, layout.focusedPaneId)?.id ??
      ...
      focusTab: (workspaceKey, tabId) => {
        ...
        set((state) => {
          ...
          const nextRoot = focusTabInPane(layout.root, pane.id, normalizedTabId);
          return {
            layoutByWorkspace: {
              ...state.layoutByWorkspace,
              [normalizedWorkspaceKey]: {
      ...
      retargetTab: (workspaceKey, tabId, target) => {
        ...
        set((state) => {
          ...
          const currentTab = collectAllTabs(layout.root).find((tab) => tab.tabId === normalizedTabId) ?? null;
          if (currentTab && tabTargetsEqual(currentTab.target, normalizedTarget)) {
            resolvedTabId = normalizedTabId;
            return state;
          }
          resolvedTabId = normalizedTabId;
          return {
            ...
            layoutByWorkspace: {
              ...
              [normalizedWorkspaceKey]: {
                root: updatePaneInTree(layout.root, {
                  ...
                  updater: (pane) => reorderTabsForPane({ pane, tabIds }),
                }),
                focusedPaneId: layout.focusedPaneId,
              },
            },
      ...
      splitPane: (workspaceKey, input) => {
        ...
        set((state) => {
          const layout = asInternalLayout(getWorkspaceLayout(state.layoutByWorkspace, normalizedWorkspaceKey));
          if (!findPaneById(layout.root, normalizedTargetPaneId)) {
            return state;
          }
          if (!findPaneContainingTab(layout.root, normalizedTabId)) {
            return state;
          }

          const result = insertSplitInternal(
            layout.root,
            normalizedTargetPaneId,
            normalizedTabId,
            input.position
          );
          if (getTreeDepth(result.root) > MAX_TREE_DEPTH) {
            return state;
          }
      ...
      moveTabToPane: (workspaceKey, tabId, toPaneId) => {
        ...
        set((state) => {
          ...
          if (!sourcePane || !findPaneById(layout.root, normalizedToPaneId)) {
            return state;
          }

          const detached = detachTabFromTree(layout.root, {
            tabId: normalizedTabId,
            preserveEmptyPaneId: sourcePane.id === normalizedToPaneId ? normalizedToPaneId : null,
          });
          if (!detached.tab) {
            return state;
          }

          const nextRoot = insertTabIntoPane(detached.root, {
            paneId: normalizedToPaneId,
            tab: detached.tab,
            focusTabId: normalizedTabId,
          });
          ...
          return {
            layoutByWorkspace: {
              ...
              [normalizedWorkspaceKey]: {
                root: nextRoot,
                focusedPaneId: normalizedToPaneId,
              },
            },
          };
        });
      ...
      resizeSplit: (workspaceKey, groupId, sizes) => {
        ...
        set((state) => {
          const layout = asInternalLayout(getWorkspaceLayout(state.layoutByWorkspace, normalizedWorkspaceKey));
          const nextRoot = updateGroupSizesInTree(layout.root, {
